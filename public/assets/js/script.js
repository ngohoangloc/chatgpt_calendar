...
function scrolled_to_bottom() {
  return (
    Math.ceil(message_list.scrollTop) + message_list.offsetHeight >=
 ...
 */
async function send_message(message_text, srole = null, smess = null) {
  ...
  data.append("srole", srole);
  data.append("smess", smess);
  ...
  eventSource.addEventListener("error", function () {
    update_message(
      message,
      "Sorry, there was an error in the request. Check your error logs."
    );
  });
  ...
  eventSource.addEventListener("message", function (event) {
    ...
    if (json.hasOwnProperty("role") && json.role === "function_call") {
      if (json.function_name === "python") {
        ...
        message.querySelector(".content").insertAdjacentHTML(
          "beforeend",
          `
...
function add_message(role, message) {
  ...
  message_item.insertAdjacentHTML(
    "beforeend",
    `
        <div class="identity">
...
function update_message(message, new_message) {
  ...
  content.querySelectorAll("pre code").forEach((el) => {
    ...
    el.appendChild(create_copy_button(code));
  });

  message.appendChild(create_copy_button(new_message));
}
...
function convert_links(text) {
  text = text.replace(
    /\(data\//g,
    "(data.php?chat_id=" + chat_id + "&file=data/"
  );
  text = text.replace(
    /\(sandbox:\/data\//g,
    "(data.php?chat_id=" + chat_id + "&file=data/"
...
$("body").on("click", ".btn-setting-item", function () {
  ...
  $.ajax({
    url: "get_setting_info.php", // URL của server để lấy thông tin
    method: "POST",
    data: {
      setting_id: settingId,
    },
    success: function (response) {
      const settingData = JSON.parse(response);

      if (settingData && settingData.setting && settingData.meta) {
        const setting = settingData.setting;
        const meta = settingData.meta;

        createConversation(setting, meta);
      } else {
        alert("Không có dữ liệu setting hoặc meta.");
      }
    },
    error: function () {
      alert("Đã có lỗi xảy ra khi lấy thông tin.");
    },
  });
});

function createConversation(setting, meta) {
  message_box.style.height = "auto";
  add_message("user", escapeHtml(meta[1].value));
  send_message(escapeHtml(meta[1].value), meta[0].setting_key, meta[0].value);
}
