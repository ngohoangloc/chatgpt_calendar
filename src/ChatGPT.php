    ...
    public function add_assistants_message(
        ...
        if (isset($googleClientConfig['web'])) {
            ...
            if (isset($_SESSION['access_token']) && $_SESSION['access_token']) {
                ...
                echo "Access token is missing or expired.";
                die();
            }

            $service = new apiCalendarService($client);

            // Lấy các sự kiện từ Google Calendar
            $calendarId = 'primary';
            $optParams = array(
                'maxResults' => 5,
                'orderBy' => 'startTime',
            ...
            if (isset($events['items']) && count($events['items']) > 0) {
                ...
                foreach ($events['items'] as $event) {
                    $eventDetails .= "Event: " . $event['summary'] . "\n";

                    // Check if 'start' has 'dateTime' or 'date'
                    if (isset($event['start']['dateTime'])) {
                        $eventDetails .= "Start: " . $event['start']['dateTime'] . "\n";
                    } elseif (isset($event['start']['date'])) {
                        $eventDetails .= "Start: " . $event['start']['date'] . " (All Day)\n";
                    }

                    // Check if 'end' has 'dateTime' or 'date'
                    if (isset($event['end']['dateTime'])) {
                        $eventDetails .= "End: " . $event['end']['dateTime'] . "\n";
                    } elseif (isset($event['end']['date'])) {
                        $eventDetails .= "End: " . $event['end']['date'] . " (All Day)\n";
                    }

                    $eventDetails .= "\n";
                }
            } else {
                $eventDetails = "Không có sự kiện nào sắp tới.\n";
            }
        }
